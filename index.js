...
async function startXeonBotInc() {
    // Update web UI with bot initialization
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            // Update message count for web UI
            if (global.updateBotState && !mek.key.fromMe) {
                global.updateBotState({
                    messagesProcessed: (global.botState?.messagesProcessed || 0) + 1,
    ...
    XeonBotInc.ev.on('connection.update', async (s) => {
        ...
        if (connection == "open") {
            ...
            // Update web UI with connection status
            if (global.updateBotState) {
                global.updateBotState({
        ...
        if (connection === 'close') {
            // Update web UI with disconnection status
            if (global.updateBotState) {
                global.updateBotState({
