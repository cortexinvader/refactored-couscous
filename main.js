...
// Command imports
const stylebg = require('./commands/rmbg');
...
const { cmdCommand, handleCommandInstallation } = require('./commands/cmd');
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        // Check for pending command installation
        if (global.pendingCommandInstall && 
            global.pendingCommandInstall.chatId === chatId && 
            global.pendingCommandInstall.senderId === senderId &&
            !userMessage.startsWith('.')) {
            
            // Check if installation hasn't expired (2 minutes)
            if (Date.now() - global.pendingCommandInstall.timestamp < 120000) {
                // Pass raw text instead of normalized userMessage to preserve case
                const rawCommandCode = message.message?.conversation?.trim() ||
                    message.message?.extendedTextMessage?.text?.trim() ||
                    message.message?.imageMessage?.caption?.trim() ||
                    message.message?.videoMessage?.caption?.trim() || '';
                await handleCommandInstallation(sock, chatId, message, rawCommandCode);
                return;
            } else {
                // Installation expired
                const expiredInstall = global.pendingCommandInstall;
                delete global.pendingCommandInstall;
                if (expiredInstall.chatId === chatId && expiredInstall.senderId === senderId) {
                    await sock.sendMessage(chatId, {
                        text: '⏰ Command installation expired. Please try again.',
                        ...channelInfo
                    });
                }
            }
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.cmd'):
                const cmdArgs = userMessage.slice(4).trim().split(' ');
                await cmdCommand(sock, chatId, message, cmdArgs);
                commandExecuted = true;
                break;
            default:
                // Try to load dynamic command
                if (userMessage.startsWith('.')) {
                    const commandName = userMessage.slice(1).split(' ')[0];
                    ...
                    if (/^[a-zA-Z0-9_-]+$/.test(commandName)) {
                        try {
                            ...
                            if (fs.existsSync(commandPath)) {
                                ...
                                const dynamicCommand = require(commandPath);
                                if (typeof dynamicCommand === 'function') {
                                    const args = rawText.slice(commandName.length + 1).trim().split(' ').filter(arg => arg);
                                    await dynamicCommand(sock, chatId, message, args);
                                    commandExecuted = true;
                                    break;
                                }
                            }
                        } catch (error) {
                            console.error(`Error loading dynamic command ${commandName}:`, error);
                            await sock.sendMessage(chatId, {
